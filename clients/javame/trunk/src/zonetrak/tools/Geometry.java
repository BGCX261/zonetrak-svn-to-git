...
/*
 * (C) 2004 - Geotechnical Software Services This code is free software; you can
 * redistribute it and/or modify it under the terms of the GNU Lesser General
 * Public License as published by the Free Software Foundation; either version
 * 2.1 of the License, or (at your option) any later version. This code is
...
public final class Geometry
{
	...
	public static double computeAngle(double[] p0, double[] p1, double[] p2) {
		...
		double angle = MathME.acos(product);
	 ...
	 */
	public static int findLineSegmentIntersection(double x0, double y0, double x1, double y1, double x2, double y2, double x3,
	...
	public static int[] createEllipse(int[] x, int[] y) {
		...
		int[] ellipse = createEllipse(x0, y0, (int) MathME.round(dx), (int) MathME.round(dy));
				...
				&& !Geometry.equals(axisAngle, 0.0, 0.1))
		{
			...
			for (int i = 0; i < nPoints; i++)
				ellipse[i * 2 + 0] += MathME.round((ellipse[i * 2 + 1] - y0) * xShear);
		}
		...
		if (ddx == 0 && ddy == 0)
			...
			angle = MathME.atan((double) ddy / (double) ddx);
		...
		for (int i = 0; i < nPoints; i++)
		{
			int xr = (int) MathME.round(x0 + (ellipse[i * 2 + 0] - x0) * cosAngle - (ellipse[i * 2 + 1] - y0) * sinAngle);
			int yr = (int) MathME.round(y0 - (ellipse[i * 2 + 1] - y0) * cosAngle - (ellipse[i * 2 + 0] - x0) * sinAngle);
	...
	public static int[] createSector(int x0, int y0, int dx, int dy, double angle0, double angle1) {
		...
		int nPoints = (int) MathME.round(arcDistance / 15);
		...
		for (int i = 0; i < nPoints; i++)
		{
			...
			xy[index + 0] = x0 + (int) MathME.round(x);
			xy[index + 1] = y0 - (int) MathME.round(y);
	...
	public static int[] createArrow(int[] x, int[] y, double length, double angle, double inset) {
		...
		// Polyline angle
		double v = dx == 0.0 ? Math.PI / 2.0 : MathME.atan(Math.abs(dy / dx));
		...
		arrow[0] = x0 + (int) MathME.round(edgeLength * Math.cos(v0));
		arrow[1] = y0 - (int) MathME.round(edgeLength * Math.sin(v0));

		arrow[4] = x0 + (int) MathME.round(edgeLength * Math.cos(v1));
		arrow[5] = y0 - (int) MathME.round(edgeLength * Math.sin(v1));

		double c1 = inset * length;

		arrow[6] = x0 + (int) MathME.round(c1 * Math.cos(v));
		arrow[7] = y0 - (int) MathME.round(c1 * Math.sin(v));
	...
	public static int[] createStar(int x0, int y0, int innerRadius, int outerRadius, int nArms) {
		...
		for (int i = 0; i < nArms * 2; i++)
		{
			...
			xy[i * 2 + 0] = (int) MathME.round(x);
			xy[i * 2 + 1] = (int) MathME.round(y);
		}
	...
	public static boolean findPolygonPosition(int[] x, int[] y, double length, int[] position) {
		...
		for (int i = 1; i < x.length; i++)
		{
			...
			if (legLength + accumulatedLength >= length)
			{
				...
				position[0] = (int) MathME.round(x[i - 1] + fraction * (x[i] - x[i - 1]));
				position[1] = (int) MathME.round(y[i - 1] + fraction * (y[i] - y[i - 1]));
				return true;
			}
